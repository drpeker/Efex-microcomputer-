...
;EFEX INLINE ASSEMBLER V.001 - 22.03.2017
;SYSTEM VARIABLES:
...
;3FB3-3FB4  :CODE BUF. LENGHT				;CODEBUFLEN
	...
	STACKPOINT .EQU  0F800H	;2 BYTE ; UNCOMMENT THIS TO RUN IN ROM
;
;
...
;
;
;	.ORG 0038H               ;CHANGEABLE INTERRUPT SUBROUTINE
;	JP 2038H
;	
;-------------------------------------------------------------------------------------
...
GREET:	             ;READY SEND FILE MSG
	LD HL,TABLE
	LD B,61   ; DECIMAL IF NOT H AT THE END
DIS:	LD A,(HL)
	CALL TXD
        INC HL
	DJNZ DIS
...
HELP:	             ;HELP MSG
	LD HL,TABLE21
	LD BC,278   ; DECIMAL IF NOT H AT THE END
DIS21:	LD A,(HL)
	CALL TXD
        INC HL
	DEC BC
...
USEFUL:	             ;HELP MSG
	...
	LD HL,TABLE22
	LD B,66   ; DECIMAL IF NOT H AT THE END
	CALL PRINT
...
DIS32:	CALL RXD
	...
	;AND 0DFH       ;%11011111       ; lower to uppercase
...
BITTI: 	LD A,0FH         
	...
	JP INTERPRET	;JUMT TO INTERPRETER
...
;        if operand=$: result in ADRSINP-3F44h                           |
;	 if operand=#: result in DATAINP                                 |
;		                                                         |
;-------------------------------------------------------------------------	
...
;EFEX INLINE ASSEMBLER V.001 - 22.03.2017
;SYSTEM VARIABLES:
;3F60-3F6F  :COMMAND INPUT BUFFER
;3FA0-3FA1  :COMMAND LINE LENGHT TO BE TRANSLATED
;3F80-3F90  :CODE BUFFER----
;3FA2-3FA3  :MNEMONIC TABLE FFINDED INDEX ADRESS
;3FA4-3FA5  :ASSEMBLY ADRESS
;3FA6-3FA7  :ASSEMBLY ADRES + CODES TO WRITE MEMORY
;3FB0	    :OPERAND BYTE COUNT
;3FB2	    : # OR $ OR NULL
;3F42       :# CONTENT (DATA)
;3F43-3F44  :$ CONTENT (ADRESS)
;3FB3-3FB4  :CODE BUF. LENGHT	
...
;---ASK WITH INPUT DISASSEMBLE ADRESS HERE---
ASKDIS: LD A,'D'
	CALL TXD
	LD A,'-'
	CALL TXD 
...
DISASK:	CALL RXD
	...
	CALL INTLINR  ;ADRESS AT 3F40 AND 3F41
	LD HL,(KEYINRES)
	LD (ADRSINP),HL ; 3F43 STORES RESULT ADRESS
	LD A, ':'
	CALL TXD
...
TABLECOD:
	...
	.DB '_LD SP',2CH,'$',1,031H,0FFH 	;KENDÝ YERÝNDE ÇALIÞMADI BU AQ. KODU :(
...
;3F60-3F6F  :COMMAND INPUT BUFFER
;3FA0-3FA1  :COMMAND LINE LENGHT TO BE TRANSLATED
;3F80-3F90  :CODE BUFFER-----NOT IN USE, RESERVED--------
;3FA2-3FA3  :MNEMONIC TABLE FFINDED INDEX ADRESS
;3FA4-3FA5  :ASSEMBLY ADRESS
;
;3FB0	    :OPERAND BYTE COUNT
...
;---------------------
;TABFINDAD TABLO KARSILASTIRMA ADRESI ICIN INDEX OLARAK KULLANILACAK
...
LOP15:	LD A,(HL)      ;BLOCK COPY SUBROUTINE
	...
	LD B,19   ; DECIMAL IF NOT H AT THE END
DIS14:	LD A,(HL)
	CALL TXD
        INC HL
	DJNZ DIS14
...
DIS15:	CALL RXD
	...
	LD HL, (KEYINRES) ;HL HOLDS SOURCE ADRESS FOR MOVE
...
SAVEX: 		
	...
	LD HL,TABLE13   ;READY SAVE FILE MSG
	LD B,28   ; DECIMAL IF NOT H AT THE END
DIS13:	LD A,(HL)
	CALL TXD
        INC HL
	DJNZ DIS13
...
DIS9:	CALL RXD
	...
	LD HL,(KEYINRES)   ;GIRILEN ADRESS
	LD A, (HL)
	LD (DATAINP),A    ;STORE SHOWING ADRESS CONTENT: RESULT DATA

	CALL HEXOUT    ;DISPLAY INPUTTED ADRESS CONTENT
	LD A, 08H
	CALL TXD
	LD A, 08H
	CALL TXD  ;2 BACKSPACE ON EDITING DATA
...
DIS10:	CALL RXD
	...
	CALL INLIN2  ;1 BYTE INPUT: RESULT IN 3F40
	LD A,(KEYINRES)
	LD (DATAINP),A
...
KAYIT:  LD A,(DATAINP)
	LD HL,(ADRSINP)
	LD (HL),A      ;ILK ADRESIN KAYDI TAMAM
	JP MA_IN
...
LOADERZ:
		             ; SEND FILE MSG
	LD HL,TABLE5
	LD B,25   ; DECIMAL IF NOT H AT THE END
DIS5:	LD A,(HL)
	CALL TXD
        INC HL
	DJNZ DIS5
	...
	LD B,11   ; DECIMAL IF NOT H AT THE END
DIS8:	LD A,(HL)
	CALL TXD
        INC HL
	DJNZ DIS8	
...
BYTERD:	LD	D,000h		;Set up   ;	* Get 2 ASCII chrs as hex byte	*
